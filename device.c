    ...
    )
{
	...
	DispatchTable[IRP_MJ_READ] = FilterRead;
...
)
{
	...
	PIRPLIST pIrp = (PIRPLIST)ExAllocatePoolWithTag(NonPagedPool, sizeof(IRPLIST), (ULONG)'ZXJ');
	if (pIrp)
	{
		pIrp->pContext = NULL;
		pIrp->pIrp = Irp;
		...
		pIrp->pNextIrp = pIrpList->pListFront;
		pIrpList->pListFront = pIrp;
