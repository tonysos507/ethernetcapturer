...
PIRPLISTHEAD pIrpList = NULL;
NTSTATUS CreateFilterList()
{
	DoTraceMessage(MYDRIVER_ALL_INFO, "Create IRP list\r\n");
	KeInitializeSpinLock(&kspIrpListLock);
	NTSTATUS NtStatus = STATUS_UNSUCCESSFUL;
	pIrpList = (PIRPLISTHEAD)ExAllocatePoolWithTag(NonPagedPool, sizeof(IRPLISTHEAD), (ULONG)'ZXJ');
	if (pIrpList)
	{
		pIrpList->pListFront = NULL;
	}

	return NtStatus;
}
NTSTATUS CreateFilterDevice(PDRIVER_OBJECT pDriverObject)
{
	PDEVICE_OBJECT pDeviceObject = 0;
	UNICODE_STRING usFilterDeviceName;
	UNICODE_STRING usDosFilterDeviceName;
	RtlInitUnicodeString(&usFilterDeviceName, L"\\Device\\zxj");
	RtlInitUnicodeString(&usDosFilterDeviceName, L"\\DosDevices\\zxj");
	IoCreateDevice(pDriverObject,
		...
		FALSE,
		&pDeviceObject);
	IoCreateSymbolicLink(&usDosFilterDeviceName, &usFilterDeviceName);
	pDeviceObject->Flags |= DO_BUFFERED_IO;
	pDeviceObject->Flags &= (~DO_DEVICE_INITIALIZING);
}
NTSTATUS ReleaseFilterDevice(PDRIVER_OBJECT pDriverObject)
{
	UNICODE_STRING usDosFilterDeviceName;
	RtlInitUnicodeString(&usDosFilterDeviceName, L"\\DosDevices\\zxj");
	IoDeleteSymbolicLink(&usDosFilterDeviceName);
...
NTSTATUS ReleaseFilterContext()
{
	DoTraceMessage(MYDRIVER_ALL_INFO, "Release IRP list\r\n");
	NTSTATUS NtStatus = STATUS_UNSUCCESSFUL;
	KIRQL kOldIrql;
	KeAcquireSpinLock(&kspIrpListLock, &kOldIrql);
	PIRPLIST pIrpListCurrent = pIrpList->pListFront;
	while (pIrpListCurrent)
	{
		IoSetCancelRoutine(pIrpListCurrent->pIrp, NULL);
		pIrpList->pListFront = pIrpListCurrent->pNextIrp;
		pIrpListCurrent->pfnCleanUpIrp(pIrpListCurrent->pIrp, pIrpListCurrent->pContext);
		ExFreePool(pIrpListCurrent);
		pIrpListCurrent = NULL;
		pIrpListCurrent = pIrpList->pListFront;
	}
	KeReleaseSpinLock(&kspIrpListLock, kOldIrql);
	return NtStatus;
}
...
NTSTATUS UseFilterContext(BYTE* pData, UINT32 bytes)
{
	...
	if (pIrpListCurrent)
	{
		PIRP pIrp = pIrpListCurrent->pIrp;
#ifdef _USE_BUFFER_IO_
...
#endif
#ifdef _USE_NEITHER_IO_
		DoTraceMessage(MYDRIVER_ALL_INFO, "Use IRP list\r\n");
		ExFreePool(pIrpListCurrent);
		pIrpListCurrent = NULL;
		...
		{
			__try {
				if (pIoStackIrp->Parameters.Read.Length > 0 && pIrp->UserBuffer)
				{
					ProbeForWrite(pIrp->UserBuffer, pIoStackIrp->Parameters.Read.Length, TYPE_ALIGNMENT(char));
					PCHAR pReadDataBuffer = pIrp->UserBuffer;
